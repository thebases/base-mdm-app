...
class SendDeviceInfoTask(private val context: Context) {
    ...
    private fun doInBackground(deviceInfo: DeviceInfo): Int {
        ...
        var response = try {
            serverService.sendDevice(settingsHelper.serverProject, deviceInfo).execute()
        } catch (e: Exception) {
            e.printStackTrace()
            null
        }
        ...
        return try {
            if (response == null) {
                response = secondaryServerService.sendDevice(settingsHelper.serverProject, deviceInfo).execute()
            }
