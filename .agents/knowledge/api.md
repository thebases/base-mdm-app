# Base MDM App — API Reference

> Generated from deep codebase scan. Source files: `app/src/main/java/com/base/launcher/server/`, `json/`, `db/`.
...
```
- `protocol`: extracted from server URL setting (http / https)
- `project`: path component stored separately in device settings

**Common Request Headers** (`server/ServerService.java`, `Const.java`):

| Header | Direction | Description |
|---|---|---|
| `X-Request-Signature` | Request | SHA-1 HMAC of request body |
| `X-CPU-Arch` | Request | Device CPU architecture string |
| `Content-Type: application/json` | Request | Set explicitly on POST/PUT endpoints |
| `X-IP-Address` | Response | Client's external IP returned by server |
| `X-Response-Signature` | Response | SHA-1 of response body for verification |

---

## Endpoints

All paths are relative to the base URL. `{project}` and `{number}` are path parameters injected per-request.
...
**Response**: `ResponseBody` (no typed model; success = HTTP 2xx)
...
**Special**: Read timeout overridden to 300,000 ms. Runs in `PushLongPollingService`.
...
**Purpose**: Batched log upload. Max 10 items per request. Scheduled with 1-min interval, 15-min retry on failure (`RemoteLogWorker`).
...
| `kioskHome` | boolean | Show Home button in kiosk |
| `kioskRecents` | boolean | Show Recents in kiosk |
| `kioskNotifications` | boolean | Show notifications in kiosk |
| `kioskSystemInfo` | boolean | Show system info in kiosk |
| `kioskKeyguard` | boolean | Enable keyguard in kiosk |
| `kioskLockButtons` | boolean | Lock hardware buttons in kiosk |
| `kioskScreenOn` | boolean | Keep screen on in kiosk |
...
| `kioskExit` | boolean | Allow kiosk exit |
...
| `kioskMode` | boolean | Kiosk mode active |
...
| Value | Action |
|---|---|
| `configUpdating` | Configuration update in progress |
| `configUpdated` | Configuration update complete |
| `runApp` | Launch an application |
| `uninstallApp` | Uninstall an application |
| `deleteFile` | Delete a specific file |
| `purgeDir` | Purge a directory |
| `deleteDir` | Delete a directory |
| `permissiveMode` | Enter permissive mode |
| `runCommand` | Execute a shell command |
| `reboot` | Reboot device |
| `exitKiosk` | Exit kiosk mode |
| `clearDownloadHistory` | Clear download history |
| `intent` | Send an Android intent |
| `grantPermissions` | Grant runtime permissions |
| `adminPanel` | Show admin panel |
| `updateOta` | Trigger OTA firmware update |
| `deviceAction` | Generic device action |
| `deviceBroadcast` | Send a broadcast |
| `deviceFactoryReset` | Perform factory reset |
...
| Field | Type | Description |
|---|---|---|
| `timestamp` | long | Log entry timestamp (ms) |
| `logLevel` | int | Log level (`0`=VERBOSE … `6`=ASSERT) |
| `packageId` | String | Source package |
...
| Field | Type | Description |
|---|---|---|
| `ts` | long | Snapshot timestamp (ms) |
| `device` | [Device](#detailedinfo-device) | Device state |
| `wifi` | [Wifi](#detailedinfo-wifi) | WiFi state |
| `gps` | [Gps](#detailedinfo-gps) | GPS state |
| `mobile` | [Mobile](#detailedinfo-mobile) | SIM 1 mobile state |
| `mobile2` | [Mobile](#detailedinfo-mobile) | SIM 2 mobile state |
...
| `useKiosk` | boolean | Run in kiosk mode |
...
| `remove` | boolean | Delete file flag |
| `description` | String | File description |
| `varContent` | boolean | Content varies per device |
...
**Source**: Task implementation files (`GetServerConfigTask`, `SendDeviceInfoTask`, etc.)
...
response.isSuccessful()                         // HTTP 2xx
  ...
  → read response.headers().get("X-IP-Address") // Optional: capture external IP
```
...
| HTTP 4xx / 5xx | `!response.isSuccessful()` branch; log and retry |
| `status != "OK"` | Application error; specific keys like `"error.notfound.device"` trigger re-enrollment |
| Invalid response signature | Signature mismatch → drop response, log security warning |
| No response body | Null body → treated as failure |

### Signature verification (when enabled)

```
sha1 = CryptoHelper.getSHA1String(REQUEST_SIGNATURE + responseBodyJson)
assert sha1 == response.headers().get("X-Response-Signature")
```

### Retry strategy

1. Attempt primary server URL
2. On failure, attempt secondary URL (if configured)
3. Background execution via WorkManager / Service with proper error propagation
...
| `PushLongPollingService` | `GET /notification/polling/{n}` | Continuous (5-min read timeout, reconnects immediately) |
